  ...
  <div class="stats" v-show="pokemon">
    <div class="screen">
      ...
      <div
        ...
        v-bind:key="item.type.name"
      >
        <span class="pokemon-type">{{ item.type.name }}</span>
      </div>
    </div>
    <ul class="pokemon-stats">
      <img v-bind:src="pokemon.sprites.back_shiny" class="image-stat" />
      <stat
        v-bind:item="item"
...
.pokemon-stats {
  background: rgba(45, 122, 180, 1);
  background-color: linear-gradient(
    111deg,
    #284e61 0%,
    rgba(45, 122, 180, 1) 3%,
    #284e61 5%,
    rgba(45, 122, 180, 1) 15%,
    rgba(45, 122, 180, 1) 17%,
    #284e61 40%,
    rgba(45, 122, 180, 1) 49%,
    #284e61 98%,
    rgba(45, 122, 180, 1) 100%
  );
...
.in-btn {
  ...
  &--tag {
    ...
    position: relative;
    top: 40px;
    animation: lights 1s alternate infinite;
    &.secundary {
      background-color: #ff651b;
      &:hover {
        background-color: red;
      }
      animation: lights 2s reverse infinite;
    }
    &:hover {
      cursor: pointer;
      color: #000;
      background-color: #05ff34;
    }
    &:checked {
      color: yellow;
    }
